package com.basistheory.elements.example.viewmodel
...
open class ApiViewModel(application: Application) : AndroidViewModel(application) {
    ...
    fun encryptToken(encryptTokenRequest: EncryptTokenRequest): LiveData<Any> = liveData {
        _errorMessage.value = null
        _result.value = null

        runCatching {
            bt.encryptToken(encryptTokenRequest)
        }.fold(
            onSuccess = {
                _result.value = it.prettyPrintJson()
                emit(it)
